...
Сбор, разметка и реалтайм-оповещения доведены до команд и описаны; офлайн-аналитика — кластеризация, роли каналов, стиль письма — живёт отдельными скриптами в [`notebooks/`](notebooks/), которые считают по базе и складывают результат в xlsx, в интерактивную карту и в markdown-отчёты по отдельным каналам.
...
- Тесты **не ходят в сеть** — Telethon замокан, фикстуры синтетические. Работают на отдельных базах, чьи имена оканчиваются на `_test`; создаёт и удаляет их фикстура.
- `make test` гоняет их в восемь процессов (`pytest-xdist`), по своей базе на воркер: `itgraph_gw0_test`, `itgraph_gw1_test` и так далее. Схема в каждой строится один раз за прогон, между тестами таблицы чистятся `TRUNCATE`. Прогон целиком — около 50 секунд вместо пяти минут. Убитый прогон оставляет свои базы, следующий их подберёт: он удаляет перед тем, как создать. Читать упавший тест удобнее в один процесс — `make test WORKERS=0`.
