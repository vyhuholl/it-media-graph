...
**Проект в работе.** Сбор и разметка каналов доведены до команд и описаны, аналитика пока живёт отдельными скриптами в [`notebooks/`](notebooks/), которые считают по базе и складывают результат в xlsx.

## Как устроено

- **Сбор идёт через MTProto (Telethon) от лица обычного аккаунта.** Bot API не умеет читать историю каналов, поэтому бота здесь нет и быть не может. Каналы читаются по username, вступать в них не нужно и вредно: массовые вступления — самый сильный сигнал бана.
