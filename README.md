...
**Проект в работе.** Сбор, разметка и реалтайм-оповещения доведены до команд и описаны; офлайн-аналитика — кластеризация, роли каналов, стиль письма — живёт отдельными скриптами в [`notebooks/`](notebooks/), которые считают по базе и складывают результат в xlsx и в интерактивную карту.
...
| `data/` | результаты аналитики. Таблицы и GEXF в git не идут — в них есть колонки разметки; карта кластеров идёт, в ней только seed-каналы и граф между ними |
